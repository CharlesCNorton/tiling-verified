...
(** ** Substitutional derivability for [QGLP_form].

    [qupd sigma p chi] rebinds the propositional variable [p] to [chi];
    it is the same update the semantics [QGLP_eval] performs at a
    quantifier.  [QGLP_derives] tracks [QGLP_eval] clause for clause,
    carrying [|-] where the semantics carries [classical_valid]: a
    universally quantified body must be derivable under every instance
    of its variable, an existentially quantified body under at least
    one.  Both quantifier clauses constrain the body, so neither is
    dischargeable without a derivation of the instances. *)

Definition qupd (sigma : nat -> Form) (p : nat) (chi : Form) : nat -> Form :=
  fun v => if Nat.eqb v p then chi else sigma v.

Inductive QGLP_derives : (nat -> Form) -> QGLP_form -> Prop :=
  | QDer_modal : forall sigma phi,
      |- subst_form sigma phi -> QGLP_derives sigma (Q_modal phi)
  | QDer_forall : forall sigma p q,
      (forall chi, QGLP_derives (qupd sigma p chi) q) ->
      QGLP_derives sigma (Q_forall p q)
  | QDer_exists : forall sigma p chi q,
      QGLP_derives (qupd sigma p chi) q ->
      QGLP_derives sigma (Q_exists p q).

Definition QGLP_provable (q : QGLP_form) : Prop := QGLP_derives Var q.
...
Proof.
  intros phi H. apply QDer_modal. rewrite subst_form_id. exact H.
Qed.

(** ** Soundness of [QGLP_derives] against [QGLP_eval].

    A derivation yields validity under the same substitution, at every
    quantifier depth.  The quantifier cases go through because the two
    relations update the substitution identically. *)

Theorem QGLP_derives_sound : forall sigma q,
  QGLP_derives sigma q -> QGLP_eval sigma q.
Proof.
  intros sigma q H.
  induction H as [sigma phi Hp
                 | sigma p q Hall IH
                 | sigma p chi q Hd IH].
  - cbn. exact (provable_classically_valid _ Hp).
  - cbn. intro psi. exact (IH psi).
  - cbn. exists chi. exact IH.
Qed.

Theorem QGLP_provable_valid : forall q,
  QGLP_provable q -> QGLP_kripke_valid q.
Proof. intros q H. exact (QGLP_derives_sound Var q H). Qed.
...
(** ** The quantifier clauses separate formulas.

    The universally quantified identity is derivable; the universally
    quantified atom is not, refuted through [QGLP_derives_sound] against
    the semantic counterexample.  An existential is discharged by
    exhibiting its instance. *)

Theorem QGLP_derives_forall_id :
  QGLP_provable (Q_forall 0 (Q_modal (Impl (Var 0) (Var 0)))).
Proof.
  unfold QGLP_provable. apply QDer_forall. intro chi.
  apply QDer_modal. unfold qupd. cbn. exact (prov_id chi).
Qed.

Theorem QGLP_derives_forall_atom_unprovable :
  ~ QGLP_provable (Q_forall 0 (Q_modal (Var 0))).
Proof.
  intro H.
  exact (QGLP_forall_var_not_valid (QGLP_provable_valid _ H)).
Qed.

Theorem QGLP_derives_exists_witness :
  QGLP_provable (Q_exists 0 (Q_modal (Var 0))).
Proof.
  unfold QGLP_provable.
  apply (QDer_exists Var 0 Top (Q_modal (Var 0))).
  apply QDer_modal. unfold qupd. cbn. exact (prov_id Bot).
Qed.

Theorem qglp_genuine_summary :
  (forall sigma q, QGLP_derives sigma q -> QGLP_eval sigma q) /\
  (forall phi, |- phi -> QGLP_provable (Q_modal phi)) /\
  QGLP_provable (Q_forall 0 (Q_modal (Impl (Var 0) (Var 0)))) /\
  (~ QGLP_provable (Q_forall 0 (Q_modal (Var 0)))) /\
  QGLP_provable (Q_exists 0 (Q_modal (Var 0))).
Proof.
  split; [|split; [|split; [|split]]].
  - exact QGLP_derives_sound.
  - exact QGLP_modal_subsumption.
  - exact QGLP_derives_forall_id.
  - exact QGLP_derives_forall_atom_unprovable.
...
(** ** Carlson speedup: Type-level T_n derivations and proof lengths.

    [Bew_term n] reifies T_n-derivations; [proof_length_in_T_n] counts
    axiom leaves and rule applications.  Two soundness engines --
    [bew_forces] (a cutoff semantics giving [Bew_no_high_box]) and
    [box_atom_eval] (boxes as atoms, giving [Bew_0_no_loeb_instance]) --
    separate T_(n+1) from T_n: [Carlson_witness n] is a one-leaf axiom of
    T_(n+1) with no T_n-derivation. *)
...
(** Cutoff semantics: boxes below the cutoff range over the descending
    Fnat-style order, boxes at or above it are False. *)
...
(** Boxes-as-atoms semantics for the T_1 / T_0 boundary. *)
...
(** Witness for the T_(n+1) / T_n boundary: at [S m] the NextCon axiom
    [Box (S m) (Neg (Box m Bot))] (outermost box at level n, so not
    T_n-provable); at [0] the level-0 Loeb instance. *)
...
(** The acceptance-literal statement for n >= 1; the tower lower bound
    holds because T_n has no derivation of the witness, surfaced by
    [Carlson_separation_strict]. *)
