...
25. Implement a true PSPACE decision procedure for the full
    ...
    complexity bound.
26. Prove decidability of the full polymodal calculus via filtration
    plus 32.
...
35. Prove the existence of a finite refuting frame for every
...
41. Prove independence of `Ax_K` using non-Kripke (e.g. neighborhood)
...
60. Wrapper-content audit: for every `Theorem foo : ... Proof.
...
74. Prove the relative-consistency direction `Con(T_0) → Con(T_n)`
    ...
    full system.
75. Prove `T_no_self_consistency` directly from `Bew`'s HBL
...
83. Construct a hierarchy of partial truth predicates `Trₙ` where
    ...
    hierarchy.
84. Prove Tarski undefinability in its sharpest form: no formula
    `Tr(x)` in the language of GLP* with one free variable
    satisfies `|- Iff (Tr ⌜φ⌝) φ` for all `φ`.
85. Prove a strong undefinability theorem by Gödel diagonalisation
...
104. Replace the constant `Cooperate := ⊤` with a genuine action
...
151. Prove that the `Provable_plus` extension scheme yields
     ...
     level uniformly.
152. Prove the reverse direction of
